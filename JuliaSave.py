...
def draw(width,height,maxIterations,zoom,com):
    ...
    if (len(pixels) >= math.floor(width*height)):
        ...
        status = cv2.imwrite("C:\\Users\\deopw\\Desktop\\Carpetas Generales\\DiegoPascualPython\\mandelbrot\\julia_PNG\\julia_" +str(height)+"p_"+str(c.real)+"_"+str(c.imag)+"i"+".png" , imgBRG)
        if(status):
            print("success")
        else:
            print("fail")
...
for i in range(1):
    ...
    draw(math.floor(size*(4/3)),math.floor(size),iterations,i+resumeI,c)
